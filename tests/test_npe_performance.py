...
class BenchmarkRunner:
    ...
    def _check_thresholds(self, result: BenchmarkResult, thresholds: BenchmarkThresholds) -> bool:
        ...
        if percentiles["p50"] * 1000 > thresholds.p50_max_ms:
            print(
                f"  ⚠️  p50 ({percentiles['p50']*1000:.2f}ms) exceeds threshold ({thresholds.p50_max_ms}ms)"
            )
            passed = False

        if percentiles["p95"] * 1000 > thresholds.p95_max_ms:
            print(
                f"  ⚠️  p95 ({percentiles['p95']*1000:.2f}ms) exceeds threshold ({thresholds.p95_max_ms}ms)"
            )
            passed = False

        if percentiles["p99"] * 1000 > thresholds.p99_max_ms:
            print(
                f"  ⚠️  p99 ({percentiles['p99']*1000:.2f}ms) exceeds threshold ({thresholds.p99_max_ms}ms)"
            )
            passed = False

        if result.throughput < thresholds.min_throughput:
            print(
                f"  ⚠️  Throughput ({result.throughput:.2f} ops/sec) below threshold ({thresholds.min_throughput})"
            )
            passed = False
...
class TestProposerClientConcurrency:
    ...
    def test_concurrent_requests(
        self, benchmark_runner, mock_proposer_response, mock_repair_response
    ):
...
class TestFullBenchmarkSuite:
    ...
    def test_run_all_benchmarks(self):
        ...
        with patch("cnsc.haai.nsc.proposer_client.requests.Session") as mock_session_class:
            ...
            def propose():
                return client.propose(
                    domain="gr", candidate_type="repair", context={}, budget={"max_wall_ms": 1000}
                )
            ...
            result = runner.run(
                "Full Suite - Propose", propose, iterations=50, thresholds=thresholds
            )
            if result.throughput < thresholds.min_throughput:
                all_passed = False
        ...
        result = runner.run(
            "Full Suite - Receipt Creation", create_receipt, iterations=100, thresholds=thresholds
        )
        if result.throughput < thresholds.min_throughput:
            all_passed = False
