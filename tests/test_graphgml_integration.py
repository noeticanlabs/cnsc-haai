...
class TestCompleteWorkflow:
    ...
    def test_multi_commit_chain_workflow(self):
        ...
        paths = query.find_path(
            "commit1", "commit3", [EdgeType.SCHEDULED_AFTER.value, EdgeType.SCHEDULED_AFTER.value]
        )
...
class TestPerformance:
    ...
    def test_large_graph_query_performance(self):
        ...
        # Find path through chain
        paths = query.find_path("s99", "s0", [EdgeType.SCHEDULED_AFTER.value] * 99)
...
class TestQueryPatterns:
    ...
    def test_find_all_commits_with_proof(self):
        ...
        commits_with_proof = [
            c for c in all_commits if graph.get_neighbors(c.node_id, EdgeType.REQUIRES_PROOF)
        ]
    ...
    def test_find_state_lineage(self):
        ...
        builder = GraphBuilder()
        graph = (
            builder.add_state("s1", state_type="initial", version=1)
            .begin_commit("commit1")
