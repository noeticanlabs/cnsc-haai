...
def load_test_vector(tv_name: str) -> dict:
    ...
    vectors_dir = (
        Path(__file__).parent.parent.parent.parent / "tests" / "vectors" / "npe" / "v1_0_1"
    )
...
def test_tv1_proposal_jcs_hash():
    ...
    jcs_bytes = json.dumps(
        tv1["proposal"], separators=(",", ":"), sort_keys=True, ensure_ascii=True
    ).encode("utf-8")
...
def test_tv2_proposal_jcs_hash():
    ...
    # Compute JCS bytes
    jcs_bytes = json.dumps(
        tv2["proposal"], separators=(",", ":"), sort_keys=True, ensure_ascii=True
    ).encode("utf-8")
...
def test_tv1_prng_seed_deterministic():
    ...
    seed_preimage = (
        PRNG_SEED_PREFIX + parent_slab_hash_bytes + npe_state_hash_bytes + proposal_id_bytes
    )
