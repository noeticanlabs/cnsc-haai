...
Cert = (
    ...
    | CertVectorNormBound
    | CertMerkleInclusion
    | CertRenormQuotientSoundness
    | CertRenormUnfoldSafety
)
...
def parse_cert_payload(cert_type: int, payload: bytes) -> Cert:
    ...
    if cert_type == CertType.CERT_BOUND_Q18:
        ...
        if len(payload) != 56:  # 32 + 32 + 8 + 8 + 8 + 8
            raise ValueError(
                f"CERT_RENORM_QUOTIENT_SOUNDNESS: expected 56 bytes, got {len(payload)}"
            )
...
def parse_op(opcode: int, flags: int, arg_bytes: bytes) -> Any:
    ...
    if opcode == Opcode.Z_SPARSE_ADD:
        ...
        for _ in range(bias_k):
            ...
            offset += 12
        return OpZAffineDiagBias(
            diag_k=diag_k, diag_entries=diag_entries, bias_k=bias_k, bias_entries=bias_entries
        )
        ...
        return OpMPatchPredictorAffine(
            patch_id=arg_bytes[0:32], cert_idx=struct.unpack(">H", arg_bytes[32:34])[0]
        )
...
def parse_delta_program(data: bytes) -> DeltaProgram:
    ...
    if op_count > MAX_OP_COUNT:
        raise ValueError(f"Delta program: exceeds max op count {MAX_OP_COUNT}, got {op_count}")
    ...
    for _ in range(op_count):
        ...
        if offset + arg_len > len(data):
            raise ValueError(
                f"Delta program: insufficient data for args (need {arg_len}, have {len(data)-offset})"
            )
