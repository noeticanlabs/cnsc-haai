...
def parse_delta_a(data: bytes) -> DeltaA:
    ...
    if len(data) < 1:
        raise ParseError(
            f"DELTA_A: insufficient data for kind: "
            f"got {len(data)} bytes, need at least 1"
        )
    ...
    if len(data) > offset:
        raise ParseError(
            f"DELTA_A: trailing bytes after parse: "
            f"got {len(data)} bytes, parsed up to {offset}"
        )
...
def _parse_atlas_entries(data: bytes) -> List[AtlasEntry]:
    ...
    if len(data) < 2:
        raise ParseError(
            f"Atlas: insufficient data for num_entries: "
            f"got {len(data)} bytes, need at least 2"
        )
    ...
    for _ in range(num_entries):
        ...
        if offset + 1 > len(data):
            raise ParseError(
                f"Atlas: insufficient data for entry_type at offset {offset}"
            )
        ...
        if offset + 2 > len(data):
            raise ParseError(
                f"Atlas: insufficient data for payload_len at offset {offset}"
            )
...
def _parse_cert_entries(data: bytes) -> List[CertEntry]:
    ...
    if len(data) < 2:
        raise ParseError(
            f"Cert: insufficient data for num_certs: "
            f"got {len(data)} bytes, need at least 2"
        )
    ...
    for _ in range(num_certs):
        ...
        if offset + 1 > len(data):
            raise ParseError(
                f"Cert: insufficient data for cert_type at offset {offset}"
            )
        ...
        if offset + 2 > len(data):
            raise ParseError(
                f"Cert: insufficient data for cert_len at offset {offset}"
            )
...
def parse_delta(data: bytes) -> Delta:
    ...
    if len(data) < 1:
        raise ParseError(
            f"Delta: insufficient data for format byte: "
            f"got {len(data)} bytes, need at least 1"
        )
    ...
    if format_byte == 0:
        ...
        raise ParseError(
            f"Delta: unknown format byte: {format_byte}"
        )
