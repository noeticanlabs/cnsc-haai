...
class BudgetAccounting:
    ...
    def add_proposer_time(self, proposer_id: str, ms: int) -> None:
        ...
        Args:
            ...
            ms: Milliseconds to add
        """
        self.proposer_wall_ms[proposer_id] = (
            self.proposer_wall_ms.get(proposer_id, 0) + ms
        )
    ...
    def is_within_budget(self, budget: "Budgets") -> bool:
        ...
        """
        return (
            self.wall_ms_used <= budget.max_wall_ms and
            self.candidates_generated <= budget.max_candidates and
            self.evidence_retrieved <= budget.max_evidence_items and
