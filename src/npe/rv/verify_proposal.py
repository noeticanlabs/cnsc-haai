...
def verify_jcs_canonicalization(proposal: dict) -> Optional[VerificationError]:
    ...
    try:
        ...
        return VerificationError(code="JCS_ERROR", message=f"JCS canonicalization failed: {e}")
...
def verify_delta_hash(proposal: dict) -> Optional[VerificationError]:
    ...
    if not expected_hash:
        return VerificationError(code="MISSING_DELTA_HASH", message="delta_hash field is required")
    ...
    try:
        ...
        return VerificationError(
            code="DELTA_DECODE_ERROR", message=f"Failed to decode delta_bytes: {e}"
        )
...
def verify_delta_parse(proposal: dict) -> Optional[VerificationError]:
    ...
    try:
        ...
        return VerificationError(code="DELTA_PARSE_ERROR", message=f"Failed to parse delta: {e}")
...
def verify_budget_arithmetic(proposal: dict) -> Optional[VerificationError]:
    ...
    if not budget:
        return VerificationError(code="MISSING_BUDGET", message="budget_post is required")
    ...
    for field in ["max_steps", "max_cost", "max_debits", "max_refunds"]:
        ...
        if value is None:
            return VerificationError(
                code="MISSING_BUDGET_FIELD", message=f"budget_post.{field} is required"
            )
...
def verify_cert_hash(proposal: dict) -> Optional[VerificationError]:
    ...
    if not expected_hash:
        return VerificationError(code="MISSING_CERT_HASH", message="cert_hash field is required")
    ...
    try:
        ...
        return VerificationError(code="CERT_DECODE_ERROR", message=f"Failed to decode certs: {e}")
        ...
        if isinstance(delta, DeltaZ):
            ...
            for d in delta.deltas:
                if not is_valid_q18(d):
                    return VerificationError(
                        code="INVALID_DELTA_VALUE", message=f"Delta value out of Q18 range: {d}"
                    )
...
def verify_proposal(proposal: dict) -> VerificationOutput:
    ...
    return VerificationOutput(
        result=VerificationResult.ACCEPT, reason="All verification checks passed"
    )
