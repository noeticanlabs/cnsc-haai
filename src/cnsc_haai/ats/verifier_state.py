...
class SlabVerifierState:
    ...
    def step_finalize(
        self, claimed_window_end: int, current_height: int, min_budget: int, current_budget: int
    ) -> tuple[bool, str]:
        """
        ...
        if current_height < self.window_end:
            return (
                False,
                f"REJECT: premature, height {current_height} < window_end {self.window_end}",
            )
    ...
    def can_delete(self) -> bool:
        ...
        return (
            self.state == SlabState.FINALIZED
            and self.deletion_authorized
            and self.dispute_status != DisputeStatus.DISPUTED
        )
