...
class MinimalKernel:
    ...
    def __init__(
        ...
        self.receipt_system = ReceiptSystem(
            signing_key="cnhaai-prototype-key",
            storage_type="in_memory",
            retention="session"
        )
        self.coherence_budget = CoherenceBudget(
            current=self.settings.get("coherence_budget", 0.5)
        )
    ...
    def execute_episode(
        ...
        state = {
            "coherence_budget": self.coherence_budget.current
        }
        ...
        return EpisodeResult(
            episode_id=episode_id,
            success=success,
            final_phase=self.phase_manager.current_phase.name if self.phase_manager.current_phase else "unknown",
    ...
    def _execute_phase(
        ...
        for result in results:
            if result.decision == GateDecision.FAIL:
                ...
                self.coherence_budget.degrade(amount=0.02, reason=f"gate_warning_{result.gate_type.name}")
