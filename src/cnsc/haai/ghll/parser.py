...
class Span:
    ...
    def __add__(self, other: "Span") -> "Span":
        """Combine two spans."""
        return Span(start=self.start, end=other.end, source=self.source or other.source)

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary."""
        return {"start": list(self.start), "end": list(self.end), "source": self.source}
...
class Token:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary."""
        return {"type": self.token_type.name, "value": self.value, "span": self.span.to_dict()}
    ...
    def tokenize(self) -> List[Token]:
        ...
        while not self._at_end():
            self._current_span = Span(
                start=(self.line, self.col), end=(self.line, self.col), source=self.source_name
            )
...
class GHLLParser:
    ...
    def _parse_return_stmt(self) -> Dict[str, Any]:
        ...
        return {"type": "return_statement", "value": value, "span": span.to_dict()}
    ...
    def _parse_assert_stmt(self) -> Dict[str, Any]:
        ...
        return {"type": "assert_statement", "condition": condition, "span": span.to_dict()}
    ...
    def _parse_primary(self) -> Optional[Dict[str, Any]]:
        ...
        if token.token_type == TokenType.IDENTIFIER:
            self._advance()
            return {"type": "identifier", "name": token.value, "span": token.span.to_dict()}
    ...
    def _error(
        ...
        error = ParseError(
            message=message, span=span, expected=expected, found=found, can_recover=can_recover
        )
