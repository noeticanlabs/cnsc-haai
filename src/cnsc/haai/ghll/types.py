...
class StructType(CompositeType):
    ...
    def __init__(
        ...
        super().__init__(
            name=name,
            element_types=list(fields.values()),
            type_id=type_id,
            origin=origin
        )
        self.fields = fields
...
class UnionType(CompositeType):
    ...
    def is_compatible_with(self, other: GHLLType) -> bool:
        ...
        if isinstance(other, UnionType):
            ...
            return all(
                any(v1.is_compatible_with(v2) for v2 in other.variants)
