...
class Genesis:
    ...
    def verify(self, receipt: Receipt) -> bool:
        ...
        return (
            receipt.previous_receipt_id == GENESIS_RECEIPT_ID
            and receipt.chain_hash == GENESIS_CHAIN_HASH
        )
...
class ReceiptVerifier:
    ...
    def verify_step(
        ...
        error = self._verify_budget_law(budget_before, budget_after, receipt, kappa)
        if error:
            return False, error
    ...
    def _verify_state_hash_before(self, state: State, receipt: Receipt) -> Optional[ATSError]:
        """
    ...
    def _verify_state_hash_after(self, state: State, receipt: Receipt) -> Optional[ATSError]:
        """
    ...
    def _verify_risk_values(
        self, state_before: State, state_after: State, receipt: Receipt
    ) -> Optional[ATSError]:
        """
        ...
        if actual_risk_before != expected_risk_before:
            return RiskMismatchError(expected_risk_before.to_json(), actual_risk_before.to_json())

        # Verify risk after
        expected_risk_after = receipt.content.risk_after_q
        if actual_risk_after != expected_risk_after:
            return RiskMismatchError(expected_risk_after.to_json(), actual_risk_after.to_json())
        ...
        if delta_plus != expected_delta_plus:
            return RiskMismatchError(expected_delta_plus.to_json(), delta_plus.to_json())

        return None

    def _verify_budget_law(
        self, budget_before: QFixed, budget_after: QFixed, receipt: Receipt, kappa: QFixed
    ) -> Optional[ATSError]:
        """
        ...
        risk_delta = compute_signed_delta(
            receipt.content.risk_after_q, receipt.content.risk_before_q
        )
    ...
    def verify_trajectory(
        ...
        for i, receipt in enumerate(receipts):
            ...
            if content.state_hash_before != expected_state_hash:
                return False, StateHashMismatchError(expected_state_hash, content.state_hash_before)
            ...
            if receipt.previous_receipt_id != self._previous_receipt_id:
                return False, InvalidChainLinkError(
                    self._previous_receipt_id, receipt.previous_receipt_id
                )
...
def verify_step(
    ...
    return default_verifier.verify_step(
        state_before, state_after, action, receipt, budget_before, budget_after, kappa
    )
...
def verify_trajectory(
    ...
    """
    return default_verifier.verify_trajectory(initial_state, initial_budget, receipts, kappa)
