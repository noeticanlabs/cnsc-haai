...
class CommitmentClock(BaseClock):
    ...
    def compute_dt(
        ...
        recent_changes = sum(
            ...
            if intent.get("stability", 1.0) < 0.5
        )
    ...
    def compute_dt(
        ...
        for delta in proposal.delta_ops:
            if hasattr(delta, "prerequisites"):
                ...
                unmet = [
                    prereq for prereq in delta.prerequisites
...
class DriftClock(BaseClock):
    ...
    def compute_dt(
        ...
        # Check tag changes
        tag_deltas = sum(
            1 for delta in proposal.delta_ops
            if delta.operation in (
                DeltaOperationType.ADD_TAG,
                DeltaOperationType.REMOVE_TAG
            )
        )
