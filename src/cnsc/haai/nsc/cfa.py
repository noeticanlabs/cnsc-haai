...
class CFATransition:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'CFATransition':
        ...
        return cls(
            ...
            phase=CFAPhase[data["phase"]] if isinstance(data["phase"], str) else CFAPhase(data["phase"]),
...
class CFAAutomaton:
    ...
    def transition_to(
        ...
        # Check phase transition rules
        if not from_state.phase.can_transition_to(to_state.phase):
            return False, f"Invalid phase transition: {from_state.phase.to_string()} -> {to_state.phase.to_string()}"
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "transitions": {
                k: [t.to_dict() for t in v] for k, v in self.transitions.items()
            },
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'CFAAutomaton':
        ...
        automaton = cls(
            ...
            name=data["name"],
            default_phase=CFAPhase[data["default_phase"]] if isinstance(data["default_phase"], str) else CFAPhase(data["default_phase"]),
        ...
        automaton.phase_history = [
            (datetime.fromisoformat(t), CFAPhase[p])
            for t, p in data.get("phase_history", [])
        ]
