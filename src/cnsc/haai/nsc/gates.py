...
class GateCondition:
    ...
    def evaluate(self, context: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if self.condition_type == "threshold":
            ...
            return (
                passed,
                f"{self.name}: {actual_value} {'>=' if self.operator is None else self.operator} {self.threshold}",
            )
...
class EvidenceSufficiencyGate(Gate):
    ...
    def evaluate(self, context: Dict[str, Any]) -> GateResult:
        ...
        # Make decision
        required_failed = any(not c.is_required or c.evaluate(context)[0] for c in self.conditions)
...
class GateManager:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pass_rate": (
                self.total_passes / self.total_evaluations if self.total_evaluations > 0 else 1.0
            ),
        }
