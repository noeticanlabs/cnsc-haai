    ...
    def check_coupling(
        ...
        for thread in active_threads:
            ...
            if coherence < self.min_coherence:
                return (
                    False,
                    f"Thread {thread.name} coherence too low: {coherence} < {self.min_coherence}",
                )
...
class PhaseLoom:
    ...
    def add_coupling(self, coupling: ThreadCoupling) -> None:
        ...
        if from_thread:
            if (
                coupling.coupling_type == "depends_on"
                and coupling.to_thread not in from_thread.depends_on
            ):
    ...
    def to_graph(self) -> Optional[core.GraphGML]:
        ...
        for coupling_id, coupling in self.couplings.items():
            ...
            combined.add_edge(
                f"{coupling.from_thread}_thread", "has_coupling", coupling_node.node_id
            )
            combined.add_edge(coupling_node.node_id, "couples_to", f"{coupling.to_thread}_thread")
...
class PhaseLoomThread:
    ...
    def to_graph(self) -> Optional[core.GraphGML]:
        ...
        for idx, event in enumerate(self.events):
            ...
            if idx > 0:
                prev_event_id = self.events[idx - 1].get(
                    "event_id", f"{self.thread_id}_event_{idx - 1}"
                )
                graph.add_edge(f"state_{prev_event_id}", "scheduled_after", state_node.node_id)
        ...
        for receipt_id, receipt_list in self.receipts.items():
            for receipt in receipt_list:
                ...
                if prev_receipt_id:
                    graph.add_edge(
                        f"commit_{prev_receipt_id}", "requires_proof", commit_node.node_id
                    )
    ...
    def query_causal_path(self, start_event_id: str, end_event_id: str) -> Optional[List[str]]:
        """
