...
class HadamardMatrix:
    ...
    def __post_init__(self):
        ...
        if len(self.matrix) != self.order:
            raise ValueError(f"Matrix order {self.order} doesn't match actual size {len(self.matrix)}")
    ...
    @classmethod
    def create_by_order(cls, hadamard_order: HadamardOrder, version: str = "1.0") -> 'HadamardMatrix':
...
class ErrorDetector:
    ...
    def decode(self, codeword: List[int]) -> Tuple[List[int], bool]:
        ...
        max_idx = transformed.index(max_val) if max_val in transformed else -transformed.index(-max_val)
...
class HadamardCodec:
    ...
    @classmethod
    def create_by_order(cls, hadamard_order: HadamardOrder, version: str = "1.0") -> 'HadamardCodec':
