...
def fit_loglog_linear(sizes: list, x_min_pct: float = 0.1, x_max_pct: float = 0.9) -> Tuple[float, float, float]:
    """
...
def fit_power_law_mle(sizes: List[float], x_min: float = None) -> Tuple[Optional[float], Optional[float]]:
    """
...
def fit_exponential_mle(sizes: List[float], x_min: float = None) -> Tuple[Optional[float], Optional[float]]:
    """
...
def run_statistical_analysis(sizes: List[float]) -> Dict:
    ...
    print(f"p-value (power-law vs exponential): {pvals['p_powerlaw_vs_exponential']:.4f}" 
          if pvals['p_powerlaw_vs_exponential'] else "N/A")
    print(f"p-value (power-law vs log-normal): {pvals['p_powerlaw_vs_lognormal']:.4f}"
          if pvals['p_powerlaw_vs_lognormal'] else "N/A")
    print()
    ...
    significant_vs_exp = pvals['p_powerlaw_vs_exponential'] is not None and pvals['p_powerlaw_vs_exponential'] < 0.05
    significant_vs_ln = pvals['p_powerlaw_vs_lognormal'] is not None and pvals['p_powerlaw_vs_lognormal'] < 0.05
...
def main():
    ...
    for seed in [123, 456]:
        ...
        if r['alpha_hill']:
            print(f"  Alpha = {r['alpha_hill']:.3f}, Slope = {r['slope']:.3f}, R² = {r['r_squared']:.3f}")
    ...
    print()
    print(f"p-value (power-law vs exponential): {result['p_powerlaw_vs_exponential']:.4f}" 
          if result['p_powerlaw_vs_exponential'] else "N/A")
    print(f"p-value (power-law vs log-normal): {result['p_powerlaw_vs_lognormal']:.4f}"
          if result['p_powerlaw_vs_lognormal'] else "N/A")
    print()
    print("=" * 70)
